...
from search.variants.qdrant_pdf.indexer import COLLECTION, MODEL, SPARSE_KWARGS, SPARSE_MODEL, connect_store, API_KEY
    ...
    def __init__(self, *, top_k: int = 10, rrf_weights: list[float] = (0.4, 0.6)) -> None:
        ...
        if self._store.count_documents() == 0:
            raise IndexNotReadyError(
                f"Kolekcja {COLLECTION} jest pusta. Zbuduj indeks: uv run search index qdrant_pdf"
            )
        self._top_k = top_k
