...
from search.loaders import load_program_pdfs, recursive_split
from settings import settings

MODEL = settings.openai_embedding_model
SPARSE_MODEL = "Qdrant/bm25"
SPARSE_KWARGS = {"disable_stemmer": True}
COLLECTION = settings.qdrant_collection_pdf
...
class QdrantPDFIndexer(Indexer):
    ...
    def build(self) -> None:
        docs = load_program_pdfs()
        if not docs:
            sys.exit(f"Brak plików .pdf w {settings.programy_pdf_dir} (rozpakuj data_rag/programy_pdf.zip).")
...
def build_indexer() -> QdrantPDFIndexer:
    return QdrantPDFIndexer()
